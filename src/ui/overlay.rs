    ...
    let popup = match &app.overlay {
        ...
        Overlay::ConfirmDiscard(_) => Some((
            ...
            vec![
                ...
                String::new(),
                "[y/Enter/w] yes    [n] no, discard    [Esc] cancel".to_string(),
            ],
            62,
        )),
        Overlay::Batch(view) => {
...
fn help_lines() -> Vec<String> {
    [
        ...
        "                  hugging — e.g. 10:00, +10s (from THIS marker's own",
        "                  position), ++10s/--10s (from start/end), 50%",
        "  i               same as b/e, for whichever marker is active",
        "  c               same jump, for the cursor alone (+/- relative to",
        "                  the cursor itself) — doesn't change which marker",
        "                  is hugging",
        ...
        "  Ctrl-↑/↓, C-k/j next / previous song in this folder",
        "  a               show every tag the file carries, not just the",
        "                  preconfigured fields (preconfigured order, then",
        "                  the rest alphabetically); press again to go back",
